...
static inline float complex calculate_root(const int x_cord, const int y_cord, complex float *roots)
{
    ...
    // This could be changed to 50 instead of 128 (för konvergensbilden ja men inte för attractorbilden)
    ...
    void *args)
{
    ...
    for (int ix = tx; ix < sz; ix += istep)
    {
        ...
        for (int jx = 0; jx < sz; ++jx)
        {
            ...
            attractors_ix[jx] =  (5479*ix*jx+3919*ix+7043*jx) % 10;        //För att göra lite roligare bilder
            convergences_ix[jx] = (5479*ix*jx+3919*ix+7043*jx) % 127;      //För att göra lite roligare bilder
        }
