...
main()
{
  ...
  {
    FILE *clfp = fopen("./dotprod_float4.cl", "r");
    if ( clfp == NULL ) {
  ...
  input_buffer_a = clCreateBuffer(context, CL_MEM_READ_ONLY, sz*sizeof(cl_float4), NULL, &error);
  if ( error != CL_SUCCESS ) {
    fprintf(stderr, "cannot create buffer a\n");
    return 1;
  }
  input_buffer_b = clCreateBuffer(context, CL_MEM_READ_ONLY, sz*sizeof(cl_float4), NULL, &error);
  if ( error != CL_SUCCESS ) {
    fprintf(stderr, "cannot create buffer b\n");
    return 1;
  }
  output_buffer_c = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sz*sizeof(cl_float), NULL, &error);
  if ( error != CL_SUCCESS ) {
    fprintf(stderr, "cannot create buffer c\n");
    return 1;
  }

  cl_float4 *a = malloc(sz*sizeof(cl_float4));
  cl_float4 *b = malloc(sz*sizeof(cl_float4));
  for ( int ix = 0; ix < sz; ++ix ) {
    a[ix] = (cl_float4){ix, ix, ix, ix};
    b[ix] = (cl_float4){ix, ix, ix, ix};
  }
  if ( clEnqueueWriteBuffer(command_queue,
           input_buffer_a, CL_TRUE, 0, sz*sizeof(cl_float4), a, 0, NULL, NULL)
       != CL_SUCCESS ) {
    fprintf(stderr, "cannot enqueue write of buffer a\n");
    return 1;
  }
  if ( clEnqueueWriteBuffer(command_queue,
           input_buffer_b, CL_TRUE, 0, sz*sizeof(cl_float4), b, 0, NULL, NULL)
       != CL_SUCCESS ) {
  ...
  float *c = malloc(sz*sizeof(cl_float));
  if ( clEnqueueReadBuffer(command_queue,
           output_buffer_c, CL_TRUE, 0, sz*sizeof(cl_float), c, 0, NULL, NULL)
       != CL_SUCCESS ) {
  ...
  printf("(Four times the) Dot product equals %f\n", c_sum);
